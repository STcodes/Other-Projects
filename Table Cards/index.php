...
<body>
    ...
    <section class="container" style="background-color:transparent;">
            ...
            <?php
                ...
                $db = Database::connect();
                $a = $db->query('SELECT DISTINCT card.date FROM card ORDER BY card.date DESC');
                while($d = $a->fetch()){
                    ...
                    $statement =$db->prepare("SELECT card.id, card.code,client.name,card.table,card.heured,card.heuref,card.evenement FROM card LEFT JOIN client ON card.client = client.id WHERE card.date = ? ORDER BY card.table ASC");
                    $statement->execute(array($d['date']));
